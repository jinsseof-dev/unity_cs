    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Detail[] arrDetails =
            {
                ...
                new Detail() { _name = "Juliet", gender = 0 }, // Student에 없음
            ...
            var QueryData =
                ...
                join detail in arrDetails on data._name equals detail._name           // 내부 조인
                //join detail in arrDetails on data._name equals detail._name into inData // 외부 조인
                //from detail in inData.DefaultIfEmpty(new Detail() { gender = 1 })       // 외부 조인
                select new
                {
                    name = data._name,
            ...
            {
                Console.WriteLine("data.name: {0}", data.name);
                Console.WriteLine("data.total: {0}", data.total);
                Console.WriteLine("data.gender: {0}", data.gender);
                Console.WriteLine();
            }
        }
    }
